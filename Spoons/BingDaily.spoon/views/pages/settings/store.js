...
const BingSettingsStore = {
  install(app) {
    const state = {
      form: Vue.reactive({
        intervalHours: 3, saveDir: "", archiveDays: 7, autoApply: true,
        notifyEnabled: true, applyToScreens: "main",
      }),
      saved: Vue.ref(false),
    ...
    const actions = {
      ...
      save() {
        ...
        return hsFetch('/settings', {
          ...
          setTimeout(() => { state.saved.value = false; }, 2000);
        });
      },
      ...
      refresh() {
        ...
        return hsFetch('/refresh', { method: 'POST' }).then(() => actions.refreshStatus()).catch((e) => { alert('刷新失败: ' + e.message); })
          .finally(() => { state.busy.value = ''; });
      },
      applyToday() {
        state.busy.value = 'applyToday';
        return hsFetch('/apply-today', { method: 'POST' }).then((d) => {
      ...
      applyRandom() {
        ...
        return hsFetch('/random', { method: 'POST' }).then((d) => {
          alert('已随机应用壁纸: ' + (d.path || ''));
          return actions.refreshStatus();
        }).catch((e) => { alert('随机应用失败: ' + e.message); })
          .finally(() => { state.busy.value = ''; });
      },
      openDir() {
        state.busy.value = 'openDir';
        return hsFetch('/open-dir', { method: 'POST' }).then((d) => {
          alert('已打开保存目录: ' + (d.dir || ''));
      ...
      refreshStatus() {
        return hsFetch('/status').then((d) => {
          state.status.value = d || {};
        }).catch(() => {});
      },
      loadDownloads() {
        return hsFetch('/downloads').then((d) => {
