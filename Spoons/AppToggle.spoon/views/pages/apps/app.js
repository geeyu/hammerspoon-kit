...
createApp({
  setup() {
    ...
    function openEditor(a) { store.openEditor(a); }

    function onHotkey(val) { store.onHotkey(val); }

    function saveEditor() { store.saveEditor(); }
    ...
    function press(a) { store.press(a); }
    ...
    return { apps: state.apps, editorOpen: state.editorOpen, editor: state.editor,
             saving: state.saving, noWindowOptions,
             openEditor, onHotkey, saveEditor, remove, press, clearLayouts, goBack,
             fmtHotkey: store.fmtHotkey };
  },
}).use(AppToggleStore).mount("#app");
