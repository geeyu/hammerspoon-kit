...
(() => {
  ...
  HSUI.initGlassFX = () => {
    ...
    function drift(el) {
      ...
      var a = anime(Object.assign({
        targets: el,
        duration: 18000 + Math.random() * 22000,
        alternate: true, loop: true,                          // v4: direction:'alternate' → alternate:true
      ...
      var tw = anime({
        targets: el, opacity: [base * 0.3, base],
        duration: 2200 + Math.random() * 3400,
        alternate: true, loop: true,
        ease: 'inOutSine'
      });
      tw.seek(tw.duration * Math.random());
    }
    var i;
    for (i = 0; i < 8; i++) drift(makeOrb('', 2 + Math.random() * 3, (0.35 + Math.random() * 0.35).toFixed(2)));
    for (i = 0; i < 5; i++) drift(makeOrb('fx-orb--bokeh', 12 + Math.random() * 12, (0.08 + Math.random() * 0.12).toFixed(2)));
  };
