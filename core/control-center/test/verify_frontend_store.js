...
const calls = [];
let fetchImpl = null;
function mockFetch(fn) { fetchImpl = fn; }
...
vm.runInContext(fs.readFileSync('core/control-center/views/pages/control-center/store.js', 'utf8') + '\n;this.__storeExport = ControlCenterStore;', sandbox);
const ControlCenterStore = sandbox.__storeExport;

let provided = {};
ControlCenterStore.install({ provide: (k, v) => { provided = v; } });
const actions = provided;

// ---- providerUrl ----
const p1 = { name: 'a', pages: [{ name: 'x', configUrl: '/a/view/pages/x/index.html' }], cards: [] };
check('configUrl 优先', actions.providerUrl(p1) === '/a/view/pages/x/index.html');

const p2 = { name: 'b', pages: [{ name: 'x' }], cards: [{ key: 'k', kind: 'page', url: '/b/view/pages/y/index.html' }] };
check('无 configUrl 取 page 卡 url', actions.providerUrl(p2) === '/b/view/pages/y/index.html');

const p3 = { name: 'c', pages: [], cards: [{ key: 'k', kind: 'openurl', url: 'https://x' }] };
...
// ---- load(): GET /providers ----
calls.length = 0;
mockFetch(() => Promise.resolve({ ok: true, json: () => Promise.resolve({ providers: [{ name: 'e' }] }) }));
actions.load();
setTimeout(() => {
  check('load GET 路径', calls.length === 1 && calls[0].url === '/control-center/api/providers');
  check('load 填充 providers', actions.state.providers.value.length === 1 && actions.state.providers.value[0].name === 'e');
