...
本仓库把「工具开发」拆成三个可独立理解的部分：**核心框架（core/hsutil）**、**命令中枢（core/launcher）**、**工具包（Spoons/*.spoon）**。开发新工具 = 用框架写一个 Spoon + 注册 Launcher 命令卡片。
...
├── launcher-commands.lua    # Launcher 协议（可选，见下）
└── test/                    # 测试
```
...
在 Spoon 根目录放 `launcher-commands.lua`：

```lua
-- 启动时 Launcher 自动扫描 Spoons/*.spoon/ 下的该文件
return {
    name = "mytool",
    cards = {
        ["我的工具"] = {
            description = "一句话说明",
            kind = "page",          -- page=面板子页面 / 其他 kind 见 core/launcher/README.md
            icon = "🔧",
...
重启 Hammerspoon 后，`Option+Space` 主页即出现「我的工具」卡片。
...
- `core/launcher/README.md` — 命令中枢完整能力与协议说明
